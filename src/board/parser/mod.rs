...
#[derive(Clone, Serialize, Deserialize)]
pub struct PartiT {
    pub cabecera : Vec<(String)>,           // serán los campos de RegistrobD
    pgn : String,
    movims: Vec<String>,                // es simplemente el array de las jugadas solamente
    pub arr_partida : Vec<Vec<MoveT>>,      // es toda la partida. Cada movim. es del tipo moveT
    pub partida_json: String,
}
...
pub fn procesa_jugadas (pgn_txt: String) -> Vec<PartiT>{
    ...
    // quitamos los \n de las jugadas, dejamos los \n de la cabecera
    let bloques: Vec<&str> = pgn_txt.split("\n\n").collect();
    let jugadas: String = bloques[1].replace("\n", " ").to_string();
    //let con_fen = format!("[FEN \"{}\"]", fen);
    let cab_elems: Vec<&str> = bloques[0].split("\n").collect();
    //let mut hay_cab_fen = false;
    for cab in cab_elems {
      let elem = cab.clone().to_string();
      //if elem.contains("FEN") {
      //  hay_cab_fen = true;
      //}
      lineas_partida.push(elem);
    }
    //if !hay_cab_fen {
    //  lineas_partida.push(con_fen);
    //}
    //lineas_partida.push("\n".to_string());
    lineas_partida.push(jugadas);
    ...
    let retorno = arr_todas_partidas;
    retorno
}
...
fn ahorma_partida(partida: &mut PartiT, parti: &mut Vec<String>) -> PartiT{
    ...
    if campos_cabeza.contains_key(&campo) {
        if let Some(x) = campos_cabeza.get_mut(&campo) {
          // por comodidad ponemos el num de jugada inicial = 1
          let mut partes_fen: Vec<String> = x.to_string()
            .split(" ")
            .map(|s| s.to_string())
            .collect();
          //partes_fen[4] = "0".to_string();  // movim peones
          partes_fen[5] = "1".to_string();  //movim. num.
          let mut unida: String = String::new();
          for elem in partes_fen {
            if unida.len() > 0 {
              unida = format!("{} {}", unida, elem);
            }
            else {
              unida = format!("{}", elem);
            }
          }
          partida.cambia_cabecera(unida);
        }
    }
    else {
